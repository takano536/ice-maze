...
class Field:
    ...
    def __init__(self, field: list, ans: list, ans_color: dict, font: str, font_size: int, tile_size: tuple, speed_offset: float, surface: pygame.Surface) -> None:
        self.__field = field
        ...
        self.__player = modules.player.Player(self.__start_coord, tile_size, speed_offset, surface)
    ...
    def __draw_ans(self, surface: pygame.Surface, dirty_rects: list):
        if self.__is_wrote_ans and not self.__player.is_moving():
            return
        for i, l in enumerate(self.__ans):
            for j, num in enumerate(l):
                ...
                rect = ((j - 1) * self.__tile_size[1], i * self.__tile_size[0], self.__tile_size[0], self.__tile_size[1])
                surface.blit(self.__floor_img, rect)
                surface.blit(text, coord)
                dirty_rects.append(rect)
        self.__is_wrote_ans = True

    def __draw_mask(self, surface: pygame.Surface, dirty_rects: list):
        for rect in self.__player.get_mask_rects():
            i, j = self.__coord2idx(pygame.Rect(rect).topleft)
    ...
    def __coord2idx(self, coord: tuple) -> tuple:
